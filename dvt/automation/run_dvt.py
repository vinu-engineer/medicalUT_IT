...
Reference: dvt/DVT_Protocol.md (DVT-001-REV-A)
"""
...
def get_text(hwnd, ctrl_id):
    ...
    if not h:
        return ""
    length = SendMessageA(h, WM_GETTEXTLENGTH, 0, 0)
    if length <= 0:
        return ""
    buf = ctypes.create_string_buffer(length + 1)
    SendMessageA(h, WM_GETTEXT, length + 1, ctypes.addressof(buf))
...
def run_dvt(exe_path):
    ...
    results.record("DVT-GUI-12", "SWR-GUI-007",
    ...
    if settings:
        ...
        results.record("DVT-GUI-13", "SWR-GUI-007",
        ...
        results.record("DVT-GUI-14", "SWR-GUI-007, SWR-SEC-003",
        ...
        results.record("DVT-GUI-16", "SWR-GUI-012",
            "Language selector with 4 languages available",
            lang_count == 4,
...
def generate_report(results, exe_path, output_dir):
    ...
    lines = [
        ...
        f"  Protocol   : dvt/DVT_Protocol.md (DVT-001-REV-A)",
        "=" * 80,
        "",
